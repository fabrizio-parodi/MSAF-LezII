...
namespace {
  ...
  struct DetectorParams {
    double density        = 3.67e2;     // g/cm³ (NaI)
...
double CalculateEnergyDeposition(const TVector3& startPoint, TVector3 direction, double energy) {
    ...
    while (energy > 0.0) {
      ...
      double muCom  = muCompton.Eval(energy)*det.density;
      double muPho  = muPhoto.Eval(energy)*det.density;
      double muTot  = muCom+muPho;
      double lambda = 1/muTot; 

      double interactionLength = -lambda*log(randomGenerator->Rndm());
	...
	break;
      ...
      bool isPhotoelectric = (randomGenerator->Rndm()<muPho/muTot);
      if (isPhotoelectric) {
...
void RunSimulation(const std::string& mode) {
	...
	if (depositedEnergy > 0.0) {
	  double measuredEnergy = depositedEnergy;
	  measuredEnergy += randomGenerator->Gaus(0,sqrt(0.02*0.02+0.04*0.04/depositedEnergy));
